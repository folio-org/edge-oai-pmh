...
class ResumptionTokeUtilsTest {
  ...
  void testFetchFrom(RequestType requestType) {
    assertEquals(DATE_FROM, ResumptionTokenUtils.fetchFrom(requestType));
  }
  ...
  private static Stream<RequestType> provideRequestTypes() {
    var token = String.format("tenantId=%st&metadataPrefix=%s&from=%s&until=%s", TENANT, OAI_DC, DATE_FROM, DATE_UNTIL);
    var encodedToken = Base64.getUrlEncoder().encodeToString(token.getBytes()).split(KEY_VALUE_DELIMITER)[0];
