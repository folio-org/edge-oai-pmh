...
public class OaiPmhHandler extends Handler {
  ...
  protected void handle(RoutingContext ctx) {
    ...
    handleCommon(ctx, null, null, (okapiClient, param) -> {
      ...
      configurationService.getEnableOaiServiceConfigSetting(configurationsClient)
        .thenAccept(enableOaiService -> {
          if (!Boolean.parseBoolean(enableOaiService)) {
            failureResponse(ctx, SC_SERVICE_UNAVAILABLE, "OAI-PMH service is disabled");
            return;
          }
          if (!supportedAcceptHeaders(request)) {
            handleNotAcceptableError(ctx, request);
          ...
          configurationService.getErrorsProcessingConfigSetting(configurationsClient)
            .thenAccept(errorsProcessing -> {
              errorsProcessingConfigSetting = errorsProcessing;
  ...
  private void badRequest(RoutingContext ctx, String body, String verb, OAIPMHerrorcodeType type) {
    ...
    handleError(ctx, SC_BAD_REQUEST, resp);
  }
